...
```{r export}
folder_path <- "/cloud/project/location-mention"
file_path <- file.path(folder_path,"loc_shot_hunt.csv")
write.csv(loc_shot_hunt, file = file_path, row.names = FALSE)
```
...
loc_read_letter <- loc_df_entry %>%
  filter(str_detect(string = journal_entry, pattern = "Read a letter|read a letter|Reed a letter|reed a letter|reed letter|read letter|Read letter|Reed letter"), ignore.case = TRUE)
...
loc_farm <- loc_df_entry %>%
  filter(str_detect(string = journal_entry, pattern = "farm|Farm"), ignore.case = TRUE)
